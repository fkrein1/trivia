...
class Game extends React.Component {
  ...
  renderAnswers = () => {
    ...
    return allAnswers[currentQuestion].map((answer, ind) => (
      <button
        key={ind}
        type="button"
        ...
        {correctAnswersIndex.length !== 0 && (
          <div>
            <p className="question">
              {this.decodeHTMLEntities(trivia[currentQuestion].question)}
            </p>
            <div className="answers">{this.renderAnswers()}</div>
            {disabledQuestion && (
              <button id="next" type="button" onClick={this.nextClick}>
                Next
              </button>
            )}
          </div>
